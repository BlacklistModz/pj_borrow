...
$_title = "วันที่ติดต่อลูกค้า";
$_action = URL."admin/status/do_contact.php";

$sql->table = "borrows";
$sql->field = "id, contact_date";
...
$arr['body'] = '<div class="form-group">
					<label for="contract_date">วันที่ติดต่อลูกค้า <span class="text-red">* (ระบุปีเป็น ค.ศ.)</span></label>
					<input type="text" class="form-control DatePicker" id="contact_date" name="contact_date" value="'.(!empty($result["contact_date"]) ? date("d/m/Y", strtotime($result["contact_date"])) : "").'" placeholder="เลือกวันที่ติดต่อลูกค้า" style="background-color:#fff;" readonly>
