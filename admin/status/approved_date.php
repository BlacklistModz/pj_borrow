...
$_title = "วันที่อนุมัติวงเงิน";
$_action = URL."admin/status/do_apr_date.php";

$sql->table = "borrows";
$sql->field = "id, approved_date";
...
$arr['body'] = '<div class="form-group">
					<label for="contract_date">วันที่อนุมัติ <span class="text-red">* (ระบุปีเป็น ค.ศ.)</span></label>
					<input type="text" class="form-control DatePicker" id="approved_date" name="approved_date" value="'.(!empty($result["approved_date"]) ? date("d/m/Y", strtotime($result["approved_date"])) : "").'" placeholder="เลือกวันที่อนุมัติ" style="background-color:#fff;" readonly>
